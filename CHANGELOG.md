...
### Changed
- **タスクトレイメニューの並び順とグルーピングの整理**: スニペット/履歴操作、FIFO/LIFOモード、設定/トグル項目、アプリ情報、終了の順に論理的なセパレーターで整理しました。
- **コードベース全体の最適化と安全性向上**:
  - 低レベルキーボードフック内での Mutex 取得を完全排除し、アトミック変数によるロックフリー同期へ移行（入力遅延・フック無効化防止）。
  - クリップボード取得時の `GlobalSize` 境界チェックおよび `datetimeformat` フィルタでのマルチバイト文字安全抽出。
  - Migemo 辞書の `OnceLock` 常駐化（0ms即時起動）およびウィンドウ非表示時のワーキングセット強制解放（スタッター要因）の廃止。
  - COM 初期化の RAII ガード (`ComGuard`) およびプロセス終了時の履歴保存フラッシュ処理を追加。
